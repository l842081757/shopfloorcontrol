 ...
 */
@Controller
public class AutoFloorTarGetYesterDayCotller {
...
/*---------------****--------------楼层健康状况----------------****--------------------*/
    ...
    @RequestMapping(value = "/AllfloorYestrerdaydata",method = RequestMethod.GET)
    public  String AllfloorYestrerdaydata(@RequestParam(value = "FloorName",required = true,defaultValue = "D073F")String FloorName, Map map){
        /*获取当前时间*/
        ...
        /*夜班累加分鐘*/
        Date actiondate;
        if (Htime >= 20 && Minute >= 30 || Htime >= 21 && Minute >= 0) {
            actiondate = schedule;
        } else {
            actiondate = YesterdayDate;
        }
        /*目標時間*/
        Date TegetDate = new Date();
        /*工作时间*/
        Integer actionMinuteD = 0;
            actionMinuteD =((Htime) * 60 + Minute);
        /*数据开始时间*/
        String DataStartTime=YesterdayDate+" "+"08:30";
        /*数据结束时间*/
...
/*--------------***----------Lot-----------**---------------------*/
        ...
        /*D073FARF线体数据*/
        List<AUTOFLOOR_TARGET> autoDayT0s = autofloorService.outPutTarGetF(FloorName,"Day",FloorName,YesterdayDate,"Lotus");
        /*TTL线体数据*/
        AUTOFLOOR_TARGET autofloor_target = autofloorService.YesterDayLinenameTarGet(FloorName, "TTL", YesterdayDate, "Lotus");
    ...
    /*---------------------------- 產能狀況  --------------------------------------*/
            ...
            /*线体每天目标*/
            AUTOFLOOR_TARGET YesterTager = autofloorService.YesterDayLinenameTarGet(FloorName, lineName, YesterdayDate, "Lotus");
            BigDecimal Target = YesterTager.getdTarget();
            Integer TargetInteger=0;
            if (Target!=null){TargetInteger =Integer.parseInt(Target.toString()); }
            /*线体每小时目标*/
            BigDecimal hTarget = YesterTager.gethTarget();
            Integer hTargetInteger=0;
            if (hTarget!=null){ hTargetInteger=Integer.parseInt(hTarget.toString()); }
            /*线体实际输出数量*/
            Integer actualOut=0;
            actualOut=autofloorService.actualOutYesterDay(FloorName,lineName,YesterdayDate,schedule,"Lotus");
            /*判断实际输出数量空值赋0*/
            if (actualOut != null&&!("").equals(actualOut)){
            ...
            if (YesterTager.getWorkingHours()!=null){
            if (YesterTager.getWorkingHours().doubleValue()==19){
                if (timeUtility.Timequantum0000_0730()){
                    if(hTargetInteger!=0&&hTargetInteger!=null) {
                        Double actdataDouble = ((actionMinuteD+510)* (hTargetInteger.doubleValue()/60) )*0.5;
                        Lotdouble2+=((actionMinuteD+510)* (hTargetInteger.doubleValue()/60) )*0.5;
                    ...
                    FloatNum=0;
                    if(hTargetInteger!=0&&hTargetInteger!=null) {
                        Double actdataDouble = (1140* (hTargetInteger.doubleValue()/60) )*0.5;
                        Lotdouble2+=(1140* (hTargetInteger.doubleValue()/60) )*0.5;
                        Double actdata =  new BigDecimal(actdataDouble).setScale(2,BigDecimal.ROUND_HALF_UP).doubleValue();
                        autoFloorGet.setActualData(actdata);
                        Double actionTargert= (1140* (hTargetInteger.doubleValue()/60) )*0.5;
                        if (actionTargert>actualOut){
                            NO3List.add(autoFloorGet);
                        }
                    }else {
            ...
            }else if (YesterTager.getWorkingHours().doubleValue()==15.34){
                if (timeUtility.Timequantum0000_0530()) {
                    ...
                    FloatNum=0;
                    if(hTargetInteger!=0&&hTargetInteger!=null) {
                        Double actdataDouble = (920* (hTargetInteger.doubleValue()/60) )*0.5;
                        Lotdouble2+=(920* (hTargetInteger.doubleValue()/60) )*0.5;
                        Double actdata =  new BigDecimal(actdataDouble).setScale(2,BigDecimal.ROUND_HALF_UP).doubleValue();
                        autoFloorGet.setActualData(actdata);
                        Double actionTargert= (920* (hTargetInteger.doubleValue()/60) )*0.5;
                        if (actionTargert>actualOut){
                            NO3List.add(autoFloorGet);
                        }
                    }else {
            ...
            }else if (YesterTager.getWorkingHours().doubleValue()==16.34){
                if (timeUtility.Timequantum0000_0630()) {
                    ...
                    FloatNum=0;
                    if(hTargetInteger!=0&&hTargetInteger!=null) {
                        Double actdataDouble = (980* (hTargetInteger.doubleValue()/60) )*0.5;
                        Lotdouble2+=(980* (hTargetInteger.doubleValue()/60) )*0.5;
                        Double actdata =  new BigDecimal(actdataDouble).setScale(2,BigDecimal.ROUND_HALF_UP).doubleValue();
                        autoFloorGet.setActualData(actdata);
                        Double actionTargert= (980* (hTargetInteger.doubleValue()/60) )*0.5;
                        if (actionTargert>actualOut){
                            NO3List.add(autoFloorGet);
                        }
                    }else {
            ...
            }else if (YesterTager.getWorkingHours().doubleValue()==17.34){
                if (timeUtility.Timequantum0000_0630()) {
                    if (hTargetInteger!=null&&hTargetInteger!=0){
                        Double actdataDouble = ((actionMinuteD+570)* (hTargetInteger.doubleValue()/60) )*0.5;
                        Lotdouble2+=((actionMinuteD+570)* (hTargetInteger.doubleValue()/60) )*0.5;
                        Double actdata =  new BigDecimal(actdataDouble).setScale(2,BigDecimal.ROUND_HALF_UP).doubleValue();
                        autoFloorGet.setActualData(actdata);
                        Double actionTargert= (actionMinuteD* (hTargetInteger.doubleValue()/60) )*0.5;
                        if (actionTargert>actualOut){
                    ...
                    FloatNum=0;
                    if(hTargetInteger!=0&&hTargetInteger!=null) {
                        Double actdataDouble = (1040* (hTargetInteger.doubleValue()/60) )*0.5;
                        Lotdouble2+=(1040* (hTargetInteger.doubleValue()/60) )*0.5;
                        Double actdata =  new BigDecimal(actdataDouble).setScale(2,BigDecimal.ROUND_HALF_UP).doubleValue();
                        autoFloorGet.setActualData(actdata);
                        Double actionTargert= (1040* (hTargetInteger.doubleValue()/60) )*0.5;
                        if (actionTargert>actualOut){
                            NO3List.add(autoFloorGet);
                        }
                    }else {
...
/*--------------------**------------------Mac------------**--------------------*/
        ...
        /*D073FARF线体数据*/
        List<AUTOFLOOR_TARGET> MacautoDayT0s = autofloorService.outPutTarGetF(FloorName,"Day",FloorName,YesterdayDate,"Macan");
        /*TTL线体数据*/
       AUTOFLOOR_TARGET macautofloor_target = autofloorService.YesterDayLinenameTarGet(FloorName,"TTL",YesterdayDate,"Macan");
        /* TLL天目标*/
        ...
        for (int i = 0; i < MacautoDayT0s.size() ; i++) {
            ...
            AUTOFLOOR_TARGET YesterTager = autofloorService.YesterDayLinenameTarGet(FloorName, lineName, YesterdayDate, "Macan");
            /*线体每天目标*/
            BigDecimal Target = YesterTager.getdTarget();
            Integer TargetInteger=0;
            if (Target!=null){
            ...
            MacactualOut=autofloorService.actualOutYesterDay(FloorName,lineName,YesterdayDate,schedule,"Macan");
            /*判断实际输出数量空值赋0*/
            if (MacactualOut != null&&!("").equals(MacactualOut)){
        ...
        if (YesterTager.getWorkingHours()!=null){
            if (YesterTager.getWorkingHours().doubleValue()==19){
                if (timeUtility.Timequantum0000_0830()) {
                    if (hTargetInteger!=null&&hTargetInteger!=0){
                        Double actdataDouble =  (((actionMinuteD+510) * (hTargetInteger.doubleValue() / 60)) * 0.5);
                        Macdouble2+=((actionMinuteD+510) * (hTargetInteger.doubleValue() / 60)) * 0.5;
                        Double actdata = new BigDecimal(actdataDouble).setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue();
                        MacautoFloorGet.setActualData(actdata);
                        Double actionTargert= ((actionMinuteD+510) * (hTargetInteger.doubleValue() / 60)) * 0.5;
                    ...
                    if(hTargetInteger!=0&&hTargetInteger!=null) {
                        Double actdataDouble = ((1140 * (hTargetInteger.doubleValue()/60) )*0.5) ;
                        Macdouble2+=(1140 * (hTargetInteger.doubleValue()/60) )*0.5;
                        Double actdata =  new BigDecimal(actdataDouble).setScale(2,BigDecimal.ROUND_HALF_UP).doubleValue();
                        MacautoFloorGet.setActualData(actdata);
                        Double actionTargert= (1140* (hTargetInteger.doubleValue()/60) )*0.5;
                        if (actionTargert>MacactualOut){
                ...
                if (timeUtility.Timequantum0000_0530()) {
                    ...
                    if(hTargetInteger!=0&&hTargetInteger!=null) {
                        Double actdataDouble = (920 * (hTargetInteger.doubleValue()/60) )*0.5;
                        Macdouble2+=(920 * (hTargetInteger.doubleValue()/60) )*0.5;
                        Double actdata =  new BigDecimal(actdataDouble).setScale(2,BigDecimal.ROUND_HALF_UP).doubleValue();
                        MacautoFloorGet.setActualData(actdata);
                        Double actionTargert= (920* (hTargetInteger.doubleValue()/60) )*0.5;
                        if (actionTargert>MacactualOut){
                ...
                if (timeUtility.Timequantum0000_0630()) {
                    if (hTargetInteger!=null&&hTargetInteger!=0){
                        ...
                        Macdouble2+=((actionMinuteD+510) * (hTargetInteger.doubleValue() / 60)) * 0.5;
                        Double actdata = new BigDecimal(actdataDouble).setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue();
                        MacautoFloorGet.setActualData(actdata+200);
                        Double actionTargert= ((actionMinuteD+510) * (hTargetInteger.doubleValue() / 60)) * 0.5;
                        if (actionTargert>MacactualOut+200){
                            MacNO3List.add(MacautoFloorGet);
                        }
                    }else {
                        MacautoFloorGet.setActualData(0.00);
                    ...
                    if(hTargetInteger!=0&&hTargetInteger!=null) {
                        Double actdataDouble = (980 * (hTargetInteger.doubleValue()/60) )*0.5;
                        Macdouble2+=(980 * (hTargetInteger.doubleValue()/60) )*0.5;
                        Double actdata =  new BigDecimal(actdataDouble).setScale(2,BigDecimal.ROUND_HALF_UP).doubleValue();
                        MacautoFloorGet.setActualData(actdata);
                        Double actionTargert= (980* (hTargetInteger.doubleValue()/60) )*0.5;
                        if (actionTargert>MacactualOut){
                ...
                if (timeUtility.Timequantum0000_0730()) {
                    if (hTargetInteger!=null&&hTargetInteger!=0){
                        Double actdataDouble = ((actionMinuteD+570) * (hTargetInteger.doubleValue() / 60)) * 0.5;
                        Macdouble2+=((actionMinuteD+570) * (hTargetInteger.doubleValue() / 60)) * 0.5;
                        Double actdata = new BigDecimal(actdataDouble).setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue();
                        MacautoFloorGet.setActualData(actdata);
                        Double actionTargert= ((actionMinuteD+560) * (hTargetInteger.doubleValue() / 60)) * 0.5;
                        if (actionTargert>MacactualOut){
                            MacNO3List.add(MacautoFloorGet);
                        }
                    }else {
                    ...
                    if(hTargetInteger!=0&&hTargetInteger!=null) {
                        Double actdataDouble = (1040 * (hTargetInteger.doubleValue()/60) )*0.5;
                        Macdouble2+=(1040 * (hTargetInteger.doubleValue()/60) )*0.5;
                        Double actdata =  new BigDecimal(actdataDouble).setScale(2,BigDecimal.ROUND_HALF_UP).doubleValue();
                        MacautoFloorGet.setActualData(actdata);
                        Double actionTargert= (1040* (hTargetInteger.doubleValue()/60) )*0.5;
                        if (actionTargert>MacactualOut){
        ...
        if (macautofloor_target!=null){
            BigDecimal MacbigDecimal = macautofloor_target.getdTarget();
            BigDecimal MacHTearget = macautofloor_target.gethTarget();
            MacHTargetsum=Integer.parseInt(MacHTearget.toString());
            MacTargetsum=Integer.parseInt(MacbigDecimal.toString());
        }
        /*TLL达成率*/
...
/*-----*-----测试机台-----*-----------*/
        ...
        for (int i = 0; i < autoAll.size(); i++) {
            ...
            /*-----***----良率----***-----*/
            List<AutoFloor_Rate> autoFloor_ratesARFs = autoFloor_rateService.AutoFloorRate_YieldYesterday(YesterdayDate,schedule,FloorName, "Lotus",autoFloorGet.getLineName());
            ...
            /*-----****---------- 误测率  ----------***------*/
            List<AutoFloor_Rate> autoFloor_ratesMisdetetARFs = autoFloor_rateService.AutoFloorRate_MisdetetYesterday(YesterdayDate,schedule,FloorName, "Lotus",autoFloorGet.getLineName());
            ...
            /*-----***-------Unknown-------***--------------*/
            List<AutoFloor_Rate> autoFloor_ratesUnknownARFs = autoFloor_rateService.AutoFloorRate_UnknownYesterday(YesterdayDate,schedule,FloorName, "Lotus",autoFloorGet.getLineName());
        ...
        for (int i = 0; i < MacautoAll.size(); i++) {
            ...
            /*-----***----良率----***-----*/
            List<AutoFloor_Rate> autoFloor_ratesMacARFs = autoFloor_rateService.AutoFloorRate_YieldYesterday(YesterdayDate,schedule,FloorName, "Macan",autoFloorMacGet.getLineName());
            ...
            /*-----****---------- 误测率  ----------***------*/
            List<AutoFloor_Rate> autoFloor_ratesMisdetetMacARFs = autoFloor_rateService.AutoFloorRate_MisdetetYesterday(YesterdayDate,schedule,FloorName, "Macan",autoFloorMacGet.getLineName());
            ...
            /*-----***-------Unknown-------***--------------*/
            List<AutoFloor_Rate> autoFloor_ratesUnknownMacARFs = autoFloor_rateService.AutoFloorRate_UnknownYesterday(YesterdayDate,schedule,FloorName, "Macan",autoFloorMacGet.getLineName());
        ...
        for (int i = 0; i < autoDayT0s.size(); i++) {
            ...
            AUTOFLOOR_TARGET autofloorTargetYesterDay = autofloorService.YesterDayLinenameTarGet(FloorName,autofloorTarget.getLineName(), YesterdayDate, "Lotus");
            ...
            AutoFloor_Wait_Time autoFloor_wait_timeA = null;
                autoFloor_wait_timeA = autoFloor_roBotService.RobotAnalyzeYesterday(YesterdayDate,schedule,FloorName, autofloorTarget.gettProduct(), autofloorTarget.getLineName()+"-A");
                if (autoFloor_wait_timeA!=null) {
                    if (autofloorTargetYesterDay.getWorkingHours().doubleValue() == 19) {
                        if (timeUtility.Timequantum0000_0730()){
                    ...
                    } else if (autofloorTargetYesterDay.getWorkingHours().doubleValue() == 15.34) {
                        if (timeUtility.Timequantum0000_0530()){
                    ...
                    } else if (autofloorTargetYesterDay.getWorkingHours().doubleValue() == 17.34) {
                        if (timeUtility.Timequantum0000_0630()){
            ...
            AutoFloor_Wait_Time autoFloor_wait_timeB = null;
                ...
                autoFloor_wait_timeB = autoFloor_roBotService.RobotAnalyzeYesterday(YesterdayDate,schedule,FloorName, autofloorTarget.gettProduct(), autofloorTarget.getLineName()+"-B");
            ...
            AutoFloor_Wait_Time autoFloor_wait_timeC = null;
                ...
                autoFloor_wait_timeC = autoFloor_roBotService.RobotAnalyzeYesterday(YesterdayDate,schedule,FloorName, autofloorTarget.gettProduct(), autofloorTarget.getLineName()+"-C");
            ...
            AutoFloor_Wait_Time autoFloor_wait_timeD = null;
                ...
                autoFloor_wait_timeD = autoFloor_roBotService.RobotAnalyzeYesterday(YesterdayDate,schedule,FloorName, autofloorTarget.gettProduct(), autofloorTarget.getLineName()+"-D");
            ...
            AutoFloor_Wait_Time autoFloor_wait_timeE = null;
                ...
                autoFloor_wait_timeE = autoFloor_roBotService.RobotAnalyzeYesterday(YesterdayDate,schedule,FloorName, autofloorTarget.gettProduct(), autofloorTarget.getLineName()+"-E");
            ...
            AutoFloor_Wait_Time autoFloor_wait_timeF = null;
                ...
                autoFloor_wait_timeF = autoFloor_roBotService.RobotAnalyzeYesterday(YesterdayDate,schedule,FloorName, autofloorTarget.gettProduct(), autofloorTarget.getLineName()+"-F");
            ...
            AutoFloor_Wait_Time autoFloor_wait_timeG = null;
                ...
                autoFloor_wait_timeG = autoFloor_roBotService.RobotAnalyzeYesterday(YesterdayDate,schedule,FloorName, autofloorTarget.gettProduct(), autofloorTarget.getLineName()+"-G");
            ...
            AutoFloor_Wait_Time autoFloor_wait_timeH = null;
                ...
                autoFloor_wait_timeH = autoFloor_roBotService.RobotAnalyzeYesterday(YesterdayDate,schedule,FloorName, autofloorTarget.gettProduct(), autofloorTarget.getLineName()+"-H");
        ...
        for (int i = 0; i < MacautoDayT0s.size(); i++) {
            ...
            AutoFloor_Wait_Time autoFloor_wait_times= new AutoFloor_Wait_Time();
            autoFloor_wait_times.setLine_name(autofloorTarget.getLineName());
            AUTOFLOOR_TARGET autofloorTargetYesterDay = autofloorService.YesterDayLinenameTarGet(FloorName,autofloorTarget.getLineName(), YesterdayDate, "Macan");
            AutoFloor_Wait_Time autoFloor_wait_timeA = null;
            autoFloor_wait_timeA = autoFloor_roBotService.RobotAnalyzeYesterday(YesterdayDate,schedule,FloorName, autofloorTarget.gettProduct(), autofloorTarget.getLineName()+"-A");
            if (autoFloor_wait_timeA!=null) {
                if (autofloorTargetYesterDay.getWorkingHours().doubleValue() == 19) {
