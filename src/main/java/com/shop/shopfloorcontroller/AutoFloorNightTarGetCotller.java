 ...
 */
@Controller
public class AutoFloorNightTarGetCotller {
...
/*---------------****--------夜班------楼层健康状况----------------****--------------------*/
    ...
    @RequestMapping(value = "/AllfloorNightdata",method = RequestMethod.GET)
    public  String Allfloordata(@RequestParam(value = "FloorName",required = true,defaultValue = "D073F")String FloorName,Map map) {
        ...
        if (timeUtility.Timequantum2029_0000()) {
            ...
            EndDate = schedule;
            TegetDate = YesterdayDate;
        }

        /*数据开始时间*/
        String DataStartTime=StartDate+" "+"20:30";
        /*数据结束时间*/
        ...
        /*D073FARF线体数据*/
        List<AUTOFLOOR_TARGET> autoNightT0s = autofloorService.outPutTarGetF(FloorName, "Night",FloorName, TegetDate, "Lotus");
        /*TTL线体数据*/
        List<AUTOFLOOR_TARGET> autoNightfloor_targets = autofloorService.outPutTarGetF(FloorName, "Night", "TTL", TegetDate, "Lotus");
        /* TLL天目标*/
        int autoDayTargetsize = 0;
        for (AUTOFLOOR_TARGET autoDayT0 : autoNightT0s) {
        ...
        for (int i = 0; i < autoNightT0s.size(); i++) {
            /*每个线体数据*/
            AutoFloorGet autoFloorGet = new AutoFloorGet();
            AUTOFLOOR_TARGET autoNightT0 = autoNightT0s.get(i);
            /*线体每天目标*/
            BigDecimal Target = autoNightT0.getdTarget();
            Integer TargetInteger = 0;
            if (Target != null) {
                TargetInteger = Integer.parseInt(Target.toString());
            }
            /*线体名称*/
            String lineName = autoNightT0.getLineName();
            /*线体每小时目标*/
            BigDecimal hTarget = autoNightT0.gethTarget();
            ...
            /*线体实际输出数量*/
            Integer actualOut = autofloorService.NightactualOut(FloorName, lineName, StartDate, EndDate, "Lotus");
            /*判断实际输出数量空值赋0*/
            if (actualOut != null) {
        ...
        if (autoNightT0.getWorkingHours()!=null){
            if (autoNightT0.getWorkingHours().doubleValue() == 9.5) {
                ...
                } else if (timeUtility.Timequantum2130_0030()) {
            ...
            } else if (autoNightT0.getWorkingHours().doubleValue() == 7.67) {
                if (timeUtility.Timequantum2030_2130()) {
                    if (hTargetInteger != 0 && hTargetInteger != null    && actionMinuteD != 0) {
            ...
            } else if (autoNightT0.getWorkingHours().doubleValue() == 8.67) {

                if (timeUtility.Timequantum2030_2130()) {
                    if (hTargetInteger != 0 && hTargetInteger != null    && actionMinuteD != 0) {
                        Double actdataDouble = actionMinuteD * (hTargetInteger.doubleValue() / 60);
                        Lotdouble2 += (actionMinuteD * (hTargetInteger.doubleValue() / 60)) * 0.5;
                        Double actdata = new BigDecimal(actdataDouble).setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue();
                        autoFloorGet.setActualData(actdata);
                        Double actionTargert = (actionMinuteD * (hTargetInteger.doubleValue() / 60)) * 0.5;
        ...
        for (AutoFloorGet autoFloorGet : autoAll) {
            ...
            for (AUTOFLOOR_TARGET autofloor_target : autoNightfloor_targets) {
            ...
            if (actionMinuteD != 0 && LotA2 != 0 && HTargetsum != null && HTargetsum != 0 && Lotdouble2 != 0) {
            ...
            if (LotA2 != 0 && Targetsum != null && Targetsum != 0) {
            ...
            map.put("LotNightautoFloorNo3List", LotNightautoFloorNo3List);
            ...
            /*D073FARF线体数据*/
            List<AUTOFLOOR_TARGET> MacautoNightT0s = autofloorService.outPutTarGetF(FloorName, "Night",FloorName, TegetDate, "Macan");
            /*TTL线体数据*/
            List<AUTOFLOOR_TARGET> MacautoNightfloor_targets = autofloorService.outPutTarGetF(FloorName, "Night", "TTL", TegetDate, "Macan");
            int MacautoDaysize = 0;
            for (AUTOFLOOR_TARGET autoDayT0 : MacautoNightT0s) {
              if (autoDayT0.getdTarget()!=null){
                  if (autoDayT0.getdTarget().intValue() > 0) {
                      MacautoDaysize++;
                  }
            ...
            for (int i = 0; i < MacautoNightT0s.size(); i++) {
                /*每个线体数据*/
                AutoFloorGet MacautoFloorGet = new AutoFloorGet();
                AUTOFLOOR_TARGET MacautoDayT0 = MacautoNightT0s.get(i);
                /*线体每天目标*/
                ...
                /*线体实际输出数量*/
                Integer MacactualOut = autofloorService.NightactualOut(FloorName, lineName, StartDate, EndDate, "Macan");
                /*判断实际输出数量空值赋0*/
            ...
            for (AUTOFLOOR_TARGET macautofloor_target : MacautoNightfloor_targets) {
            ...
            if (MacTargetsum != null&&MacTargetsum !=0) {
            ...
            map.put("MacNightAutoFloorNo3List", MacNightAutoFloorNo3List);
        ...
        for (int i = 0; i < autoAll.size(); i++) {
            ...
            /*-----***----良率----***-----*/
            List<AutoFloor_Rate> autoFloor_ratesARFs = autoFloor_rateService.AutoFloorRate_YieldNight(StartDate, EndDate,FloorName, "Lotus",autoFloorGet.getLineName());
            ...
            /*-----****---------- 误测率  ----------***------*/
            List<AutoFloor_Rate> autoFloor_ratesMisdetetARFs = autoFloor_rateService.AutoFloorRate_MisdetetNight(StartDate, EndDate,FloorName, "Lotus",autoFloorGet.getLineName());
            ...
            /*-----***-------Unknown-------***--------------*/
            List<AutoFloor_Rate> autoFloor_ratesUnknownARFs = autoFloor_rateService.AutoFloorRate_UnknownNight(StartDate, EndDate,FloorName, "Lotus",autoFloorGet.getLineName());
        ...
        for (int i = 0; i < MacautoAll.size(); i++) {
            ...
            /*-----***----良率----***-----*/
            List<AutoFloor_Rate> autoFloor_ratesMacARFs = autoFloor_rateService.AutoFloorRate_YieldNight(StartDate, EndDate,FloorName, "Macan",autoFloorMacGet.getLineName());
            ...
            /*-----****---------- 误测率  ----------***------*/
            List<AutoFloor_Rate> autoFloor_ratesMisdetetMacARFs = autoFloor_rateService.AutoFloorRate_MisdetetNight(StartDate, EndDate,FloorName, "Macan",autoFloorMacGet.getLineName());
            ...
            /*-----***-------Unknown-------***--------------*/
            List<AutoFloor_Rate> autoFloor_ratesUnknownMacARFs = autoFloor_rateService.AutoFloorRate_UnknownNight(StartDate, EndDate,FloorName, "Macan",autoFloorMacGet.getLineName());
        ...
        for (int i = 0; i < autoNightT0s.size(); i++) {
            ...
            AUTOFLOOR_TARGET autofloorTarget = autoNightT0s.get(i);
        ...
        for (int i = 0; i < MacautoNightT0s.size(); i++) {
            ...
            Double LineTTLPlateRateTwo=0.0;
            Double LineTTLPlateRateThree=0.0;
            AUTOFLOOR_TARGET autofloorTarget = MacautoNightT0s.get(i);
            AutoFloor_Wait_Time autoFloor_wait_times= new AutoFloor_Wait_Time();
            autoFloor_wait_times.setLine_name(autofloorTarget.getLineName());
        ...
        return "realtime/AllFloorsNightData";
