 ...
 */
@Controller
public class AutoFloorTarGetCotller {
...
/*---------------****--------------楼层健康状况----------------****--------------------*/
    ...
    public  String Allfloordata(@RequestParam(value = "FloorName",required = true,defaultValue = "D073F")String FloorName, Map map){
        ...
        List<AUTOFLOOR_TARGET> autoDayT0s = new ArrayList<>();
        List<AUTOFLOOR_TARGET> autofloor_targets =new ArrayList<>();

        if (timeUtility.Timequantum0830_2029()){
            /* 白班 D073FARF线体数据*/
            autoDayT0s=autofloorService.outPutTarGetF(FloorName, "Day",FloorName,schedule,"Lotus");
            /*白班 TTL线体数据*/
            autofloor_targets=autofloorService.outPutTarGetF(FloorName,"Day", "TTL",schedule,"Lotus");
        }else {
            /* 夜班 D073FARF线体数据*/
            autoDayT0s=autofloorService.outPutTarGetF(FloorName,"Night",FloorName,schedule,"Lotus");
            /*夜班 TTL线体数据*/
            autofloor_targets=autofloorService.outPutTarGetF(FloorName,"Night", "TTL",schedule,"Lotus");
            TimeGNumber=0;
        }
        ...
        map.put("TimeGNumber", TimeGNumber);
...
/*--------------***----------Lot-----------**---------------------*/
        ...
        for (int i = 0; i < autoDayT0s.size() ; i++) {
            ...
            Integer actualOut=0;
            if (timeUtility.Timequantum0830_2029()){
                actualOut = autofloorService.actualOut(FloorName,lineName,schedule,"Lotus");
            }else {
                actualOut = autofloorService.HouractualOut(FloorName,lineName,StartDate,EndDate,"Lotus");
            }
            /*判断实际输出数量空值赋0*/
            if (actualOut != null){
    ...
    if (autoDayT0.getWorkingHours()!=null){
        if (timeUtility.Timequantum0830_2029()){
            ...
            if (autoDayT0.getWorkingHours().doubleValue()==9.5){
                ...
                }else if (Htime>=7&&Minute>=30||Htime>7&&Minute>0) {
                    FloatNum=0;
                    if (hTargetInteger != 0 && hTargetInteger != null && actionMinuteD != 0) {
                        Double actdataDouble =  ((570 * (hTargetInteger.doubleValue() / 60)));
                        Lotdouble2+= 570 * (hTargetInteger.doubleValue() / 60);
                ...
                }else if (Htime>=5&&Minute>=30||Htime>5&&Minute>0) {
                    FloatNum=0;
                    if (hTargetInteger != 0 && hTargetInteger != null && actionMinuteD != 0) {
                        Double actdataDouble = 460 * (hTargetInteger.doubleValue() / 60);
                        Lotdouble2+=((460 * (hTargetInteger.doubleValue() / 60))) ;
                ...
                }else if (Htime>=6&&Minute>=30||Htime>6&&Minute>0) {
                    FloatNum=0;
                    if (hTargetInteger != 0 && hTargetInteger != null   && actionMinuteD != 0) {
                        Double actdataDouble = 520 * (hTargetInteger.doubleValue() / 60);
                        Lotdouble2+=((520 * (hTargetInteger.doubleValue() / 60))) ;
        ...
        if ( actionMinuteD!=0&&Targetsum!=null&&Targetsum!=0){
...
/*--------------------**------------------Mac------------**--------------------*/
        List<AUTOFLOOR_TARGET> MacautoDayT0s = new ArrayList<>();
        List<AUTOFLOOR_TARGET> Macautofloor_targets =new ArrayList<>();
        if (timeUtility.Timequantum0830_2029()){

            /* 白班 D073FARF线体数据*/
            MacautoDayT0s=autofloorService.outPutTarGetF(FloorName,"Day",FloorName,schedule,"Macan");
            /*白班 TTL线体数据*/
            Macautofloor_targets=autofloorService.outPutTarGetF(FloorName,"Day", "TTL",schedule,"Macan");
        }else {
            /* 夜班 D073FARF线体数据*/
            MacautoDayT0s=autofloorService.outPutTarGetF(FloorName,"Night",FloorName,schedule,"Macan");
            /*夜班 TTL线体数据*/
            Macautofloor_targets=autofloorService.outPutTarGetF(FloorName,"Night", "TTL",schedule,"Macan");
        }
        ...
        for (int i = 0; i < MacautoDayT0s.size() ; i++) {
            ...
            /*线体实际输出数量*/
            Integer MacactualOut =0;
            if(timeUtility.Timequantum0830_2029()){
                MacactualOut =autofloorService.actualOut(FloorName,lineName,schedule,"Macan");
            }else {
                MacactualOut=autofloorService.HouractualOut(FloorName,lineName,StartDate,EndDate,"Macan");
            }
        ...
        if (MacautoDayT0.getWorkingHours()!=null){
            if (timeUtility.Timequantum0830_2029()) {
                if (MacautoDayT0.getWorkingHours().doubleValue() == 9.5) {
                    if (timeUtility.Timequantum0830_0930()) {
                        ...
                        FloatNum=0;
                        if (hTargetInteger != null && hTargetInteger != 0 && MacactualOut != 0 && actionMinuteD != 0) {
                            Double actdataDouble = 570 * (hTargetInteger.doubleValue() / 60);
                        ...
                        FloatNum=0;
                        if (hTargetInteger != null && hTargetInteger != 0) {
                            Double actdataDouble = 570 * (hTargetInteger.doubleValue() / 60);
                            Macdouble2 += 570 * (hTargetInteger.doubleValue() / 60);
                            Double actdata = new BigDecimal(actdataDouble).setScale(2, BigDecimal.ROUND_HALF_UP).doubleValue();
                            MacautoFloorGet.setActualData(actdata);
                ...
                if (MacautoDayT0.getWorkingHours().doubleValue()==9.5){
                    ...
                    } else if (timeUtility.Timequantum0030_0630()) {
                        Integer actionTimeh = actionMinuteD - 80;
                        if (actionTimeh > 520) {
                            actionTimeh = 520;
                        }
            ...
            System.out.println(MacautoFloorGet.getActualData()+"-----Data");
        }
        /*取Mac倒数3个线体*/
        ...
        Double MacactdataDoublesums = ((Macints2.doubleValue()+800)/Macdouble2)*100;
...
/*-----*-----测试机台-----*-----------*/
        ...
        List<AutoFloor_Rate> autoFloor_ratesARFs =new ArrayList<>();
        List<AutoFloor_Rate> autoFloor_ratesMisdetetARFs =new ArrayList<>();
        List<AutoFloor_Rate> autoFloor_ratesUnknownARFs =new ArrayList<>();
        ...
        for (int i = 0; i < autoAll.size(); i++) {
            ...
            if (timeUtility.Timequantum0830_1029()) {
                autoFloor_ratesARFs = autoFloor_rateService.AutoFloorRate_Yield08aciton(schedule,FloorName, "Lotus",autoFloorGet.getLineName());
                autoFloor_ratesMisdetetARFs = autoFloor_rateService.AutoFloorRate_Misdetet08aciton(schedule,FloorName, "Lotus",autoFloorGet.getLineName());
                autoFloor_ratesUnknownARFs = autoFloor_rateService.AutoFloorRate_Unknown08aciton(schedule,FloorName, "Lotus",autoFloorGet.getLineName());

            } else if (timeUtility.Timequantum2030_2229()) {
                autoFloor_ratesARFs = autoFloor_rateService.AutoFloorRate_Yield20aciton(schedule,FloorName, "Lotus",autoFloorGet.getLineName());
                autoFloor_ratesMisdetetARFs = autoFloor_rateService.AutoFloorRate_Misdetet20aciton(schedule,FloorName, "Lotus",autoFloorGet.getLineName());
                autoFloor_ratesUnknownARFs = autoFloor_rateService.AutoFloorRate_Unknown20aciton(schedule,FloorName, "Lotus",autoFloorGet.getLineName());

            } else {
                autoFloor_ratesARFs = autoFloor_rateService.AutoFloorRate_Yield(FloorName, "Lotus",autoFloorGet.getLineName());
                autoFloor_ratesMisdetetARFs = autoFloor_rateService.AutoFloorRate_Misdetet(FloorName, "Lotus",autoFloorGet.getLineName());
                autoFloor_ratesUnknownARFs = autoFloor_rateService.AutoFloorRate_Unknown(FloorName, "Lotus",autoFloorGet.getLineName());
            }
            /*-----***----良率----***-----*/
            ...
            Double FPYARFdoubles =0.0;
            if (YieldsARFdouble!=0){
                FPYARFdoubles =YieldsARFdouble - MisdetetARFdouble;
            }
        ...
        /*-----****---------- 直通率  ----------***------*/
        /*-----------全部Lot线体直通率-----------------*/
        Double rateFPYAllDouble=0.0;
        if (rateYieldsAlldouble!=0){
            rateFPYAllDouble=rateYieldsAlldouble-rateMisdetetAlldouble;
        }
        /*-----------TLL全部直通率保留后4位-----------*/
        ...
        /*---------------***---------- Mac-------------***---------------*/
        List<AutoFloor_Rate> autoFloor_ratesMacARFs  =new ArrayList<>();
        List<AutoFloor_Rate> autoFloor_ratesMisdetetMacARFs =new ArrayList<>();
        List<AutoFloor_Rate> autoFloor_ratesUnknownMacARFs =new ArrayList<>();
        ...
        /*数据更新区间*/
        Long currentStartTime= System.currentTimeMillis()-2*60*60*1000;
        Long currentEndTime= System.currentTimeMillis();
        Date dateStart = new Date(currentStartTime);
        Date dateEnd = new Date(currentEndTime);
        DataStartTime= df.format(dateStart);
        ...
        for (int i = 0; i < MacautoAll.size(); i++) {
            ...
            if (timeUtility.Timequantum0830_1029()) {
                DataStartTime=schedule+" "+"08:30";
                autoFloor_ratesMacARFs = autoFloor_rateService.AutoFloorRate_Yield08aciton(schedule,FloorName, "Macan",autoFloorMacGet.getLineName());
                autoFloor_ratesMisdetetMacARFs = autoFloor_rateService.AutoFloorRate_Misdetet08aciton(schedule,FloorName, "Macan",autoFloorMacGet.getLineName());
                autoFloor_ratesUnknownMacARFs = autoFloor_rateService.AutoFloorRate_Unknown08aciton(schedule,FloorName, "Macan",autoFloorMacGet.getLineName());

            } else if (timeUtility.Timequantum2030_2229()) {
                DataStartTime=schedule+" "+"20:30";
                autoFloor_ratesMacARFs = autoFloor_rateService.AutoFloorRate_Yield20aciton(schedule,FloorName, "Macan",autoFloorMacGet.getLineName());
                autoFloor_ratesMisdetetMacARFs = autoFloor_rateService.AutoFloorRate_Misdetet20aciton(schedule,FloorName, "Macan",autoFloorMacGet.getLineName());
                autoFloor_ratesUnknownMacARFs = autoFloor_rateService.AutoFloorRate_Unknown20aciton(schedule,FloorName, "Macan",autoFloorMacGet.getLineName());
            } else {

                autoFloor_ratesMacARFs = autoFloor_rateService.AutoFloorRate_Yield(FloorName, "Macan",autoFloorMacGet.getLineName());
                autoFloor_ratesMisdetetMacARFs = autoFloor_rateService.AutoFloorRate_Misdetet(FloorName, "Macan",autoFloorMacGet.getLineName());
                autoFloor_ratesUnknownMacARFs = autoFloor_rateService.AutoFloorRate_Unknown(FloorName, "Macan",autoFloorMacGet.getLineName());
            }
            /*-----***----良率----***-----*/
            ...
            Double FPYARFMacdoubles = 0.0;
            if (YieldsARFMacdouble!=0){
                FPYARFMacdoubles = YieldsARFMacdouble - MisdetetMacARFdouble;
            }
        ...
        /*-----****---------- 直通率  ----------***------*/
        /*-----------全部Lot线体直通率-----------------*/
        Double rateMacFPYAllDouble=0.0;
        if (rateMacYieldsAlldouble!=0){
            rateMacFPYAllDouble=rateMacYieldsAlldouble - rateMacMisdetetAlldouble;
        }
        /*-----------TLL全部直通率保留后4位-----------*/
    ...
    /*-------------------------Robot-------------------------------*/
        ...
        for (int i = 0; i < autoDayT0s.size(); i++) {
            ...
            if (timeUtility.Timequantum0830_1029()) {
                autoFloor_wait_timeA = autoFloor_roBotService.RobotAnalyze08(schedule,FloorName, autofloorTarget.gettProduct(), autofloorTarget.getLineName()+"-A");
                ...
                autoFloor_wait_timeA = autoFloor_roBotService.RobotAnalyze20(schedule,FloorName, autofloorTarget.gettProduct(), autofloorTarget.getLineName()+"-A");
                if (autoFloor_wait_timeA!=null) {
            ...
            if (timeUtility.Timequantum0830_1029()) {
                autoFloor_wait_timeB = autoFloor_roBotService.RobotAnalyze08(schedule,FloorName, autofloorTarget.gettProduct(), autofloorTarget.getLineName()+"-B");
                ...
                autoFloor_wait_timeB = autoFloor_roBotService.RobotAnalyze20(schedule,FloorName, autofloorTarget.gettProduct(), autofloorTarget.getLineName()+"-B");
                if (autoFloor_wait_timeB!=null) {
                    int HtimeRate=Htime-20;
                    if (HtimeRate==0){
                        HtimeRate=1;
            ...
            if (timeUtility.Timequantum0830_1029()) {
                autoFloor_wait_timeC = autoFloor_roBotService.RobotAnalyze08(schedule,FloorName, autofloorTarget.gettProduct(), autofloorTarget.getLineName()+"-C");
                ...
                autoFloor_wait_timeC = autoFloor_roBotService.RobotAnalyze20(schedule,FloorName, autofloorTarget.gettProduct(), autofloorTarget.getLineName()+"-C");
                if (autoFloor_wait_timeC!=null) {
                    int HtimeRate=Htime-20;
                    if (HtimeRate==0){
                        HtimeRate=1;
            ...
            if (timeUtility.Timequantum0830_1029()) {
                autoFloor_wait_timeD = autoFloor_roBotService.RobotAnalyze08(schedule,FloorName, autofloorTarget.gettProduct(), autofloorTarget.getLineName()+"-D");
                ...
                autoFloor_wait_timeD = autoFloor_roBotService.RobotAnalyze20(schedule,FloorName, autofloorTarget.gettProduct(), autofloorTarget.getLineName()+"-D");
                if (autoFloor_wait_timeD!=null) {
                    int HtimeRate=Htime-20;
                    if (HtimeRate==0){
                        HtimeRate=1;
            ...
            if (timeUtility.Timequantum0830_1029()) {
                autoFloor_wait_timeE = autoFloor_roBotService.RobotAnalyze08(schedule,FloorName, autofloorTarget.gettProduct(), autofloorTarget.getLineName()+"-E");
                ...
                autoFloor_wait_timeE = autoFloor_roBotService.RobotAnalyze20(schedule,FloorName, autofloorTarget.gettProduct(), autofloorTarget.getLineName()+"-E");
                if (autoFloor_wait_timeE!=null) {
                    int HtimeRate=Htime-20;
                    if (HtimeRate==0){
                        HtimeRate=1;
            ...
            if (timeUtility.Timequantum0830_1029()) {
                ...
                autoFloor_wait_timeF = autoFloor_roBotService.RobotAnalyze08(schedule,FloorName, autofloorTarget.gettProduct(), autofloorTarget.getLineName()+"-F");
                ...
                autoFloor_wait_timeF = autoFloor_roBotService.RobotAnalyze20(schedule,FloorName, autofloorTarget.gettProduct(), autofloorTarget.getLineName()+"-F");
                if (autoFloor_wait_timeF!=null) {
                    int HtimeRate=Htime-20;
                    if (HtimeRate==0){
                        HtimeRate=1;
            ...
            if (timeUtility.Timequantum0830_1029()) {
                autoFloor_wait_timeG = autoFloor_roBotService.RobotAnalyze08(schedule,FloorName, autofloorTarget.gettProduct(), autofloorTarget.getLineName()+"-G");
                ...
                autoFloor_wait_timeG = autoFloor_roBotService.RobotAnalyze20(schedule,FloorName, autofloorTarget.gettProduct(), autofloorTarget.getLineName()+"-G");
                if (autoFloor_wait_timeG!=null) {
                    int HtimeRate=Htime-20;
                    if (HtimeRate==0){
                        HtimeRate=1;
            ...
            if (timeUtility.Timequantum0830_1029()) {
                autoFloor_wait_timeH = autoFloor_roBotService.RobotAnalyze08(schedule,FloorName, autofloorTarget.gettProduct(), autofloorTarget.getLineName()+"-H");
                ...
                autoFloor_wait_timeH = autoFloor_roBotService.RobotAnalyze20(schedule,FloorName, autofloorTarget.gettProduct(), autofloorTarget.getLineName()+"-H");
                if (autoFloor_wait_timeH!=null) {
                    int HtimeRate=Htime-20;
                    if (HtimeRate==0){
                        HtimeRate=1;
