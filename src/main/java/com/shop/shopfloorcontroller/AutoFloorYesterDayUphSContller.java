 ...
 */
@Controller
public class AutoFloorYesterDayUphSContller {
    @Autowired
    ...
    /*产能状况*/
    @RequestMapping(value ="/AllfloorsYesterDayOutput",method = RequestMethod.GET)
    public  String AllfloorsOutput(@RequestParam("Linename") String Linename,@RequestParam(value = "Productname",required = false)String Productname , Map map){
        ...
        Integer UWBsum=0;
        Integer SCONDsum=0;
        SumNumber NightsumNumber = new SumNumber();
        /*夜班工站总数*/
        Integer NightDFUsum=0;
        Integer NightFCTsum=0;
        Integer NightS1sum=0;
        Integer NightS2sum=0;
        Integer NightS3sum=0;
        Integer NightCCTsum=0;
        Integer NightW1sum=0;
        Integer NightW2sum=0;
        Integer NightUWBsum=0;
        Integer NightSCONDsum=0;
        /*实时工站总量*/
        ...
        AUTOFLOOR_TARGET targetsum = autofloorTargeService.LinenameTarGet(Linename,"Day",YesterdayDate,Productname);
        AUTOFLOOR_TARGET Nighttargetsum = autofloorTargeService.LinenameTarGet(Linename,"Night",YesterdayDate,Productname);
            /*实时时段段目标值*/
        ...
        BigDecimal NighthTargets = Nighttargetsum.gethTarget();
        int NighthTarget = NighthTargets.intValue();
        Integer hTargetInteger =Integer.parseInt(hTargets.toString());
        map.put("NightHTarget",NighthTarget);

        /*线体天目标*/
        BigDecimal DTarget = targetsum.getdTarget();
        map.put("DTarget",DTarget);

        BigDecimal NightDTarget = Nighttargetsum.getdTarget();
        map.put("NightDTarget",NightDTarget);
        /*机种工站判断*/
        ...
        /*All 每小时工站数据*/
        List<AutoFloorUphS> autoFloorUphS = autoFloorUphService.WorkstationOutYesterDay(Linename,StartDate,EndDate,Floorname,Productname);
...
/*时间段实时目标*/
        ...
        if (targetsum.getWorkingHours().doubleValue()==9.5){
            TargetH10=1;
           if (timeUtility.Timequantum0030_0130()) {
       ...
       Double stopTime=((targetsum.gethTarget().doubleValue()/60)*19.8);
            SumUphTarget=(targetsum.gethTarget().intValue()/2)+stopTime.intValue()+(targetsum.gethTarget().intValue()*2)+Targetaction0030_0130+Targetaction0130_0230+Targetaction0230_0330 +Targetaction0330_0430 +Targetaction0430_0530 +Targetaction0530_0630 +Targetaction0630_0730 ;
        if (timeUtility.Timequantum0730_2029()){
            SumUphTarget=NightDTarget.intValue();
        }
        ...
        W2ActionSum =autoFloorUphService.MachinaeActionSumYesterday(Floorname, ProductnameWIFIBT2, StartDate,EndDate,Productname,Linename);
        ...
        NightsumNumber.setCCTsum(NightCCTsum);
        NightsumNumber.setDFUsum(NightDFUsum);
        NightsumNumber.setFCTsum(NightFCTsum);
        NightsumNumber.setS1sum(NightS1sum);
        NightsumNumber.setS2sum(NightS2sum);
        NightsumNumber.setS3sum(NightS3sum);
        NightsumNumber.setUWBsum(NightUWBsum);
        NightsumNumber.setW1sum(NightW1sum);
        NightsumNumber.setW2sum(NightW2sum);
        NightsumNumber.setSCONDsum(NightSCONDsum);
        ...
        map.put("NightsumNumber" , NightsumNumber);
        ...
        return "realtime/AllFloorsYesterDayOutput";
